...
import (
	...
	response_common "small-ant-parent/small-ant-commn/response.common"
)
...
// GetPage
func GetPage(model, where interface{}, out interface{}, pageIndex, pageSize uint64, totalCount *uint64, whereOrder ...response_common.PageWhereOrder) error {
	db := DB.Model(model).Where(where)
	if len(whereOrder) > 0 {
